// src/components/Overlays/EditMultaModal.jsx
...
const EditMultaModal = ({ isOpen, onClose, onSubmit, multaData }) => {
    const [formData, setFormData] = useState(initialFormData);

    // Pre-fill form when modal opens or multaData changes
    useEffect(() => {
        if (isOpen && multaData) {
            setFormData({
                aluguerId: multaData.AluguerId || '', // Match prop names from MultaCardAdmin/API data
                // Assume DataAtribuicao is the date of the infraction for editing purposes
                data: multaData.DataAtribuicao ? new Date(multaData.DataAtribuicao).toISOString().split('T')[0] : '', // Format date
                // Clean value: remove currency symbol and trim whitespace
                valor: multaData.Valor ? String(multaData.Valor).replace(/[^0-9.,]/g, '').replace(',', '.') : '',
            });
        } else if (!isOpen) {
            setFormData(initialFormData); // Reset if closed
        }
    }, [isOpen, multaData]);
    ...
    const handleSubmit = (e) => {
        ...
        // Important: Pass the original multa ID along with the updated form data
        onSubmit(multaData.multaId, formData);
    };

    if (!isOpen || !multaData) return null;
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <InputFieldLong
                            ...
                            required
                            // Potentially disable if ID shouldn't change: disabled
                        />
                    </div>
                    <div>
